...
本书从头到尾带你建一座知识库。被服务的对象是 `aishop`——一个虚构的电商后端，有订单、库存、支付、退款、风控、对账几个模块。你要给它建的那座知识库，我们叫它 `aishop-kb`。读到第 22 章，你会拥有一座完整、可自托管、能扫覆盖度、能对外提供服务、能被全团队共建、能自我评测和自检的 `aishop-kb`。
...
agent 直接读文件则能沿符号引用确定地跳转。这也是 Claude Code 公开采用"agentic 搜索"、而非预建嵌入索引的原因——让模型用 `grep`、`glob` 直接在文件系统里找。
...
一个自然的反驳是：用重叠窗口（overlap）或按 AST（抽象语法树）边界切块，能不能消除"切断结构"？能显著缓解。但这恰恰印证了向量路径在代码场景并不"免费"。据 Cursor 公开的工程说明，它这类选择向量索引的工具，会在朴素切块之上叠加语义／AST 感知的切分，并用 Merkle 树做增量重建来对抗索引漂移。
...
文件式知识随代码的 PR 一并修改、评审、打版本标签。中心化的向量索引则始终面临"索引与真实代码"的一致性漂移——代码改了、索引还没重建，agent 拿到的是旧知识。这个漂移贯穿所有与代码分离的知识形态，第 22 章会专门讲怎么治它；而文件式知识从根上就没有这个洞。
...
这些正是 Onyx（开源、可自托管的企业搜索问答系统，前身 Danswer）和 Glean（闭源的企业知识图谱平台）的主场。它们赢在跨源模糊召回，而不赢在单仓库代码理解。把它们的能力误用到"让 agent 读懂这个仓库"上，就会重演本章开头那种检索质量不升反降的局面。第 11 章对标 Onyx 时会再回到这条边界。
