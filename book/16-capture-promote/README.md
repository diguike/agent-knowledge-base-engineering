...
到上一章为止，`aishop-kb` 已经有了两条 CLI 命令：`coverage`（第 5 章，扫覆盖度、找盲区）和 `serve`（第 10 章，起知识 MCP 服务）。这两条命令服务的都是代码衍生知识——原料本就是有坐标的文件，工具把它取出来换个形态即可。
...
`last_reviewed` 取 promote 当天。一条知识被人工筛选并规整的这一刻，正是它最后一次被验证有效的时刻。这个日期是第 22 章漂移检测和 TTL 废弃机制的输入。
