...
这一章给 `aishop-kb` 加上第二条命令 `aishop-kb serve`：起一个知识 [MCP](https://modelcontextprotocol.io) 服务，让它的知识第一次能被任意 agent 跨厂商接入。
...
选它的理由单一而决定性：**它是目前唯一被主流 agent 工具共同采纳的开放协议。** 一个 MCP 服务可同时被 Claude、[Cursor](https://cursor.com)、GitHub Copilot、Codex、Windsurf、ChatGPT 接入，一次实现、处处复用，不必为每个客户端各写一套适配。
...
embedding 是把文本映射成向量的模型，召回质量首先取决于它。选型有公开榜单 [MTEB](https://huggingface.co/spaces/mteb/leaderboard)（Massive Text Embedding Benchmark）可参考，重点看它的 retrieval 子轨而非综合分。
...
示例用内存索引与词袋向量，生产的标准落地是把索引与向量落进 [pgvector](https://github.com/pgvector/pgvector)（PostgreSQL 的向量扩展）。迁移路径清晰而不改架构：
...
`aishop-kb` 现在有了 `serve` 命令，知识第一次能被任意 agent 接入。但一个服务端立刻带来两个新问题：namespace 该怎么划、谁能召回哪部分知识（权限）。第 11 章把召回边界工程化，并兑现第 2 章的承诺——把 [GitMCP](https://gitmcp.io)、[Context7](https://context7.com) 这类现成的代码衍生层直接声明为依赖，而不自建。
