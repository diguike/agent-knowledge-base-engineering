...
本章给 `aishop-kb` 补的不是产物，是两张全书地图：一张按来源把知识分成两类，划定各自的投入；一张按形态给出四级能力阶梯，交代 `aishop-kb` 会沿它一级级长到第 23 章。

## 2.1 本章你会得到什么

1. 一条分配投入的依据——知识的两类来源在获取成本上相差一个量级，据此决定哪类靠工具、哪类靠人。
2. 一条能力阶梯（文件夹 → 包化分层 → [MCP](https://modelcontextprotocol.io) 服务 → 多包分发），以及每一级该不该往上爬的可观察升级信号。
3. 一张贯穿全书的 `aishop-kb` 逐章生长地图，标清每一章给这座知识库加了什么产物——后面每次回看主线都靠它。
4. 一条常被混淆的边界：agent 记忆（[Mem0](https://mem0.ai) / [Zep](https://www.getzep.com) / [Letta](https://www.letta.com)）为什么不是知识库。
...
| 代表工具 | [GitMCP](https://gitmcp.io) · [DeepWiki](https://deepwiki.com) · [Sourcegraph](https://sourcegraph.com) | 无现成工具，靠流程与激励 |
...
|---|---|---|---|---|
| [`llms.txt`](https://llmstxt.org)（仓库自描述） | 阶段0 | 零，仓库根一个索引文件 | 单仓库、公开，任意 agent 可读 | 第 6 章 |
...
2. MCP 端点（模型上下文协议，Model Context Protocol，让 agent 跨厂商调用外部知识与工具的开放标准）把知识做成服务，Claude、[Cursor](https://cursor.com)、Copilot 等任意 agent 接一下即可用，还能按 namespace 圈定谁能召回哪部分——**依赖声明同时是召回范围过滤器**，第 11 章会展开这条核心机制。
...
把知识库当记忆做，会丢掉版本和治理；把记忆当知识库做，会为本该过期的会话状态背上不必要的治理包袱。记忆的重点是写入与遗忘，它容忍遗忘、不需要版本和 CODEOWNERS；知识库的重点是组织、分发与治理，它的正确性是硬约束，要评测（见第 21 章）、要防漂移（见第 23 章）。本书从头到尾讲的是知识库。
