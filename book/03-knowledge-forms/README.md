...
第 2 章把 `aishop-kb` 还没落一个文件时的地图先画了出来：知识分代码衍生与手写业务两类来源，沿文件夹 → 包化分层 → [MCP](https://modelcontextprotocol.io) 服务 → 多包分发这条能力阶梯一级级长大，RAG 的家在阶段2。
...
2. 代码图谱为什么是"谁调用了谁、改它影响谁"这类问题的正解，以及 [SCIP](https://github.com/sourcegraph/scip) 与 LSP 如何同源于编译器。
...
LSP 的产物是即时的、随编辑会话存在的，服务器一关就没了，无法直接当知识库来查询。SCIP（SCIP Code Intelligence Protocol，[Sourcegraph](https://sourcegraph.com) 提出的跨语言代码索引格式）解决的正是持久化这一步。
...
企业级把这条路走到极致的是 [Glean](https://www.glean.com)（一个闭源的企业知识图谱平台）：它为每家企业训练专属的语义模型，把散在各系统里的实体和关系连成一张公司级的图，对外只暴露一个统一的 MCP 端点。
