...
上一章给 `aishop-kb` 抹平了跨 agent 的格式差异。一份知识源经 [Ruler](https://github.com/intellectronica/ruler) 生成多端产物，Claude 读 `CLAUDE.md`、[Cursor](https://cursor.com) 读 `.cursor/rules`、其余工具读 `AGENTS.md`，[MCP](https://modelcontextprotocol.io) 端点则被各厂商客户端共同接入。知识的内核第一次做到了一处编写、多端可读。
...
声明式安装把安装意图写进随仓库走的配置文件，取代那份会过期的入职清单。[Claude Code](https://claude.com/claude-code) 支持在仓库的 `.claude/settings.json` 里（project scope，即这份配置随仓库走、对所有克隆者生效）声明两件事：去哪找 marketplace、默认启用哪些 plugin。
