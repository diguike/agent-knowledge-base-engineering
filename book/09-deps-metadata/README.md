...
内容留在服务端注册表里，也就是能力阶梯阶段 2 要搭的知识 [MCP](https://modelcontextprotocol.io) 服务（第 10 章）。这个订阅声明有一个关键性质：它同时就是召回范围过滤器。
...
| `type` | [Diátaxis](https://diataxis.fr) 四象限归类 | 决定读者以何种预期消费（见第 17 章） | 教程与参考混淆，读者定位错 |
...
**TTL 强制的从来不是改内容，而是一次人工确认。**这一步把新鲜度从主观感受变成可审计信号，也是第 23 章漂移检测的地基。
...
- 五字段 frontmatter（title/type/owner/last_reviewed/status）让每份知识可治理：owner 定责、last_reviewed 配分级 TTL 管新鲜度、status 管废弃，是漂移检测（第 23 章）的地基。
