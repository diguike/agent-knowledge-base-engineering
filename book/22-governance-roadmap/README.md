---
title: 第 22 章　治理与生命周期：漂移检测、废弃机制与落地路线图
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/T6QkwysnGi8qRekDvqtcfsQEnoc"
last_synced: "2026-07-07T21:00:00+08:00"
---

读到这一章，`aishop-kb` 已经会自建、能扫覆盖度、能对外提供服务、能被全团队共建、能自我评测。它的 CLI 长齐了六条命令——`coverage`、`serve`、`promote`、`check`、`extract`、`eval`。知识分 L0/L1/L2 三层，每个包带 `owner` 和 `last_reviewed` 元数据。
...
这也印证第 1 章那条论断的另一面：文件式知识随代码 PR 一起改，reviewer 当场就能看见知识和代码是否同步，天然少一层漂移。任何和代码分离的知识——中心化索引、独立文档、MCP 注册知识——都必须额外补一套漂移检测。**漂移检测不是可选的加分项，而是任何与代码分离的严肃知识库的必需品。**

### 22.2.3 度量到治理的反馈闭环

时间漂移和一致性漂移都从知识侧主动扫描。还有一条从使用侧被动发现漂移的通道，把第 21 章的度量和本章的治理接成闭环。

第 21 章用 promptfoo 判定 agent 回答对不对。其中一类失败样本特别值钱：agent 忠实引用了某条知识（faithfulness 高），但对照 ground truth 是错的。这种失败几乎必然指向一条过期或错误的知识。
...
把 owner、last_reviewed、TTL、status 合起来，每条知识就有了一条明确的生命周期，在一个状态机里流转（图 22-1）。

```mermaid
stateDiagram-v2
    [*] --> active: 入库（有 owner、last_reviewed）
    active --> 待复核: 超期未复核（今天 - last_reviewed > TTL）
    active --> 待复核: 评测判假 / 一致性漂移触发
    待复核 --> active: 复核仍有效，刷新 last_reviewed
    待复核 --> deprecated: 确认过期，标 status 并指向替代
    deprecated --> [*]: 检索层排除、下游迁移完成
```

图 22-1：知识条目的生命周期状态机。一条知识入库即 active；被 TTL 超期、评测判假或一致性漂移任一信号推入待复核；复核后要么续期回 active、要么确认过期标 deprecated 并指向替代。

这套状态机是治理能自动化的关键。没有它，知识库是一堆状态不明的文件，只增不减、真假混杂，越久越不敢信。有了它，知识库变成每条目都有明确状态、能自动淘汰过期项、能沿状态转移施加治理动作的系统。

三个进入待复核的入口——TTL 超期、评测判假、一致性漂移——恰好对应前面讲的三条发现通道，状态机把它们统一收口成同一个复核动作。
...
## 22.5 健康度看板：覆盖度、有效性与新鲜度三位一体

把全书的度量收拢，一个知识库的健康由三个正交维度共同决定，缺一不可，且都该进 CI、都该能一眼看到（图 22-2）。
...
图 22-2：知识库健康度看板。覆盖度（该有的有没有，第 5 章）、有效性（有了准不准，第 21 章）、新鲜度（过期没有，本章）三项正交，任一退步 CI 就红。
...
图 22-3：落地路线图，即全书能力阶梯的最终 recap。四阶每一级都由左边那个具体信号触发才升级，没触发就停在原地。

按阶展开：

- 个人、小团队、单仓库：停在阶段 0（第 6 章）。一个 `docs/` 文件夹加 `llms.txt` 加确定性导航，零基础设施，对几十条知识完全够用。为这个规模上 MCP 是纯负债——维护一套向量库和索引 worker，换来的检索质量还不如 `grep`。
...
全书各章逐步搭出的能力，最终组装成了书根下的 `aishop-kb/` 成品：一座完整、可自托管的知识库，加一套统一 CLI（`aishop-kb coverage / serve / promote / check / extract / eval / drift`）和一条 `kb-health.yml` CI。这七条命令，正是你从第 5 章到本章一条条加上去的。
