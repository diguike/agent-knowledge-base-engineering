...
到上一章为止，`aishop-kb` 的 CLI 已经有四条命令：`coverage` 扫覆盖度、`serve` 起知识 [MCP](https://modelcontextprotocol.io) 服务、`promote` 把本地随手记上收成共享包、`check` 跑质量门禁。这四条覆盖了从沉淀到分发的主干。
...
它们的危害不在当下，而在将来：某天有人改了其中一条、忘了另一条，两条开始各说各话，知识库从此静默分叉、持续漂移（漂移的治理见第 23 章）。
