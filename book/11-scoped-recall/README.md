...
上一章给 `aishop-kb` 装上了 `serve` 命令。它现在的 CLI 有两条命令，对外多了一个 [MCP](https://modelcontextprotocol.io) 端点：
...
3. 一张与开源企业搜索 [Onyx](https://onyx.app) 的逐维度对标表，标出手搭版本与企业级成熟形态的分界线。
4. 一段同时挂两类 MCP 端点的客户端配置：自建服务管私有业务知识，[GitMCP](https://gitmcp.io) / [Context7](https://context7.com) 管公开的代码衍生知识。
...
`aishop-kb` 的服务端现在有了两道召回边界：命名空间与权限。但同一份知识源要被 Claude、[Cursor](https://cursor.com)、Copilot 等不同端的 agent 复用，各端的接入格式并不一致。第 12 章把这一份源分发到多端 agent，讲可移植内核与分发外壳的分离。
