...
**你会亲手建出什么**：全书带你把一座知识库 `aishop-kb`（给虚构电商后端 `aishop` 建的知识库）从空目录一路建到完整可跑——分层知识包、一个知识 MCP 服务、一套逐章长出来的 CLI（`coverage` / `serve` / `promote` / `check` / `extract` / `eval` / `ablate` / `drift`）和一条健康度 CI。读完你拿到的成品就在书根 [`aishop-kb/`](aishop-kb/)，`npx tsx src/cli.ts health` 一键跑出三位一体健康度看板。就像《自己动手写 AI Agent》带你写出 Ling、《AI 评测工程》带你写出 EvalKit——这本书带你建出 `aishop-kb`。
